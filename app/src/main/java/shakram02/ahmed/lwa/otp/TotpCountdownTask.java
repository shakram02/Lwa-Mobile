...
public class TotpCountdownTask implements Runnable {
    ...
    private long getCounterValue(long time) {
        return mCounter.getValueAtTime(shakram02.ahmed.lwa.Utilities.millisToSeconds(time));
    }
    ...
    private long getTimeTillNextCounterValue(long time) {
        ...
        long nextValueStartTime = shakram02.ahmed.lwa.Utilities.secondsToMillis(mCounter.getValueStartTime(nextValue));
    ...
    private long getCounterValueAge(long time) {
        return time - shakram02.ahmed.lwa.Utilities.secondsToMillis(mCounter.getValueStartTime(getCounterValue(time)));
    }
}
